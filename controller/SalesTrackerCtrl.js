...
export const GetSalesTrackerForAssetHolder = async (req, res) => {
  try {
    const userId = req.query.userId || req.query.userUUID || req.user._id
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message: 'Forbidden — you can only access your own sales tracker',
      })
    }
    ...
    const getProductsWithType = async (
      ...
      const selects =
        '-country -requestDocument -uploadDocument -interiorColor -extras -exteriorColor -feedback -city -phoneNumber -neighbourhood -additionalDescription -developer -facilities -ratings -__v'
      let query = Model.find({ userId, isDeleted: false }).select(selects)
